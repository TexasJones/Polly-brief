...
# One accent color per topic instead of a single uniform green -- makes
# scanning faster and gives each section its own visual identity.
TOPIC_COLORS = {
    'Campaigns': '#3357A8',
    'Media': '#8E44AD',
    'AI+Policy': '#0E8C96',
...
def _section_heading(emoji, title, color=None):
    color = color or INK
    a = '<div style="font-size:15px; font-weight:700; color:' + color + '; margin-bottom:16px; font-family:' + BODY_FONT + ';">'
...
def _stat_block(number, label, url=None):
    ...
    c = '<div style="font-size:30px; font-weight:800; color:' + ACCENT + '; letter-spacing:-0.5px; font-family:' + HEADLINE_FONT + ';">'
    d = number + '</div>'
...
def _story_block(story):
    ...
    a = '<tr><td style="padding-bottom:6px;">' + heading
    b = '<div style="font-size:16px; font-weight:700; color:' + INK + '; line-height:1.4; font-family:' + HEADLINE_FONT + ';">'
    c = _esc(item.title) + '</div>' + summary_html
    d = '<a href="' + _esc(item.url) + '" target="_blank" rel="noopener noreferrer" style="font-size:13px; font-weight:700; color:' + color + '; text-decoration:none;">'
...
def _pick_top_highlight(top_stories):
    # Same story used for both the subject line (in generate_brief.py) and
    # this in-email callout, for consistency between what someone sees in
    # their inbox and what greets them right after opening.
    for story in top_stories:
        if story.item:
            return story
    return None

def _top_highlight_block(top_stories):
    story = _pick_top_highlight(top_stories)
    if not story:
        return ''
    color = _topic_color(story.section)
    item = story.item
    a = '<tr><td style="padding:0 40px 28px 40px;">'
    b = '<div style="background-color:' + color + '0D; border-left:4px solid ' + color + '; border-radius:6px; padding:18px 20px;">'
    c = '<div style="font-size:11px; font-weight:700; color:' + color + '; text-transform:uppercase; letter-spacing:0.6px; margin-bottom:6px;">'
    d = "Today's Top Story &middot; " + story.emoji + ' ' + _esc(story.section) + '</div>'
    e = '<div style="font-size:17px; font-weight:700; color:' + INK + '; line-height:1.4; font-family:' + HEADLINE_FONT + ';">'
    f = _esc(item.title) + '</div>'
    g = '<a href="' + _esc(item.url) + '" target="_blank" rel="noopener noreferrer" style="font-size:13px; font-weight:700; color:' + color + '; text-decoration:none; margin-top:8px; display:inline-block;">'
    h = 'Read More &rarr;</a>'
    i = '</div></td></tr>'
...
def render_brief(pulse, top_stories, featured_jobs, quote_text=None, quote_source=None, today=None):
    ...
    parts.append('<body style="margin:0; padding:0; background-color:' + BG + '; font-family:' + BODY_FONT + ';">')
    # Hidden preheader text -- shows next to the subject line in the inbox
    # list, before the email is opened. Padded with invisible characters so
    # email clients don't fall back to showing raw HTML/logo alt text instead.
    top_story_for_preheader = _pick_top_highlight(top_stories)
    if top_story_for_preheader:
        preheader_text = top_story_for_preheader.item.title
    else:
        preheader_text = f'{pulse.total_active} active jobs in politics & public affairs'
    parts.append('<div style="display:none; max-height:0; overflow:hidden; mso-hide:all;">')
    parts.append(_esc(preheader_text))
    parts.append('&nbsp;' * 40)
    parts.append('</div>')
    ...
    parts.append('<td style="padding-right:10px; vertical-align:middle;"><img src="data:image/png;base64,' + logo_b64 + '" width="58" height="30" alt="Polly" style="display:block;"></td>')
    parts.append('<td style="vertical-align:middle;"><div style="font-size:22px; font-weight:800; color:' + INK + '; letter-spacing:-0.3px; font-family:' + HEADLINE_FONT + ';">The Polly Brief</div></td>')
    parts.append('</tr></table>')
    parts.append('<div style="font-size:13px; color:' + MUTED + '; margin-top:6px;">' + _esc(date_label) + '</div>')
    parts.append('</td></tr>')
    parts.append(_top_highlight_block(top_stories))
    ...
    parts.append(_section_heading('\U0001F4C5', 'Election Countdown', color='#FFFFFF'))
    parts.append('<div style="font-size:32px; font-weight:800; color:#FFFFFF; font-family:' + HEADLINE_FONT + ';">' + str(days_left) + '</div>')
    ...
    parts.append('<tr><td style="padding:24px 40px 12px 40px; text-align:center;">')
    parts.append('<a href="https://thepolly.co" target="_blank" rel="noopener noreferrer" style="font-size:12px; color:' + MUTED + '; text-decoration:underline;">Know someone job hunting in politics? Forward this along.</a>')
    parts.append('</td></tr>')
    parts.append('<tr><td style="padding:12px 40px 40px 40px;">')
